...
contract BeanBag is ERC20("Vault PAW Token", "VPAW"), IBeanBag, Ownable {
  /// @notice PAW token
  IPAW public PAW;

  constructor(IPAW _PAW) public {
    PAW = _PAW;
  }
  ...
  /// @notice Mint `_amount` BEAN to `_to`. Must only be called by MasterPAW.
  ...
  /// @notice Burn `_amount` BEAN to `_from`. Must only be called by MasterPAW.
  ...
  /// @notice Safe PAW transfer function, just in case if rounding error causes pool to not have enough PAWs.
  /// @param _to The address to transfer PAW to
  /// @param _amount The amount to transfer to
  function safePAWTransfer(address _to, uint256 _amount) external override onlyOwner {
    uint256 PAWBal = PAW.balanceOf(address(this));
    if (_amount > PAWBal) {
      PAW.transfer(_to, PAWBal);
    } else {
      PAW.transfer(_to, _amount);
    }
  }
