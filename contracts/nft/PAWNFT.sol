...
contract PAWNFT is IPAWNFT, ERC721PausableUpgradeable, OwnableUpgradeable, AccessControlUpgradeable {
  ...
  modifier onlyGovernance() {
    require(hasRole(GOVERNANCE_ROLE, _msgSender()), "PAWNFT::onlyGovernance::only GOVERNANCE role");
    _;
  }

  /// @dev only the one having a MINTER_ROLE can continue an execution
  modifier onlyMinter() {
    require(hasRole(MINTER_ROLE, _msgSender()), "PAWNFT::onlyMinter::only MINTER role");
    _;
  }
  ...
  function initialize(string memory _baseURI) public initializer {
    ERC721Upgradeable.__ERC721_init("PAW Tribute NFT", "PTNFT");
  ...
  function tokenURI(uint256 _tokenId)
    ...
    returns (string memory)
  {
    require(_exists(_tokenId), "PAWNFT::tokenURI:: token not existed");
    ...
    onlyExistingCategoryId(_newCategoryId)
  {
    ...
    require(
      ...
      "PAWNFT::updateTokenCategory::tokenId not found"
    );
    require(_categoryToPAWNFTList[_newCategoryId].add(_tokenId), "PAWNFT::updateTokenCategory::duplicated tokenId");
  ...
  function mint(
    ...
    require(_categoryToPAWNFTList[_categoryId].add(newId), "PAWNFT::mint::duplicated tokenId");
    _mint(_to, newId);
  ...
  function _setTokenURI(uint256 _tokenId, string memory _tokenURI) internal virtual override {
    require(_exists(_tokenId), "PAWNFT::_setTokenURI::tokenId not found");
  ...
  function mintBatch(
    ...
    require(_size != 0, "PAWNFT::mintBatch::size must be granter than zero");
