  ...
  IMasterPAWCallback
{
  ...
  // Map the PAWName for a tokenId
  mapping(uint256 => string) public override pawNames;

  mapping(uint256 => Category) public override categoryInfo;

  mapping(uint256 => uint256) public override pawNFTToCategory;

  mapping(uint256 => EnumerableSetUpgradeable.UintSet) private _categoryToPAWNFTList;
  ...
  event SetPAWName(uint256 indexed tokenId, string prevName, string newName);
  ...
  function initialize(
    ...
    ERC721Upgradeable.__ERC721_init("Master NFT", "PMNFT");
  ...
  modifier withOGOwnerToken(uint256 _tokenId) {
    require(
      address(ogOwnerToken[pawNFTToCategory[_tokenId]]) != address(0),
  ...
  function categoryToPAWNFTList(uint256 _categoryId)
    external
    view
    override
  ...
  {
    uint256[] memory tokenIds = new uint256[](_categoryToPAWNFTList[_categoryId].length());
    for (uint256 i = 0; i < _categoryToPAWNFTList[_categoryId].length(); i++) {
      tokenIds[i] = _categoryToPAWNFTList[_categoryId].at(i);
    }
    return tokenIds;
  }
    ...
    returns (string memory)
  {
    ...
    if (bytes(categoryInfo[pawNFTToCategory[_tokenId]].categoryURI).length > 0) {
      return string(abi.encodePacked(base, categoryInfo[pawNFTToCategory[_tokenId]].categoryURI));
    }
  ...
  {
    uint256 categoryIdToBeRemovedFrom = pawNFTToCategory[_tokenId];
    pawNFTToCategory[_tokenId] = _newCategoryId;
    require(
      _categoryToPAWNFTList[categoryIdToBeRemovedFrom].remove(_tokenId),
      "OGNFT::updateTokenCategory::tokenId not found"
    );
    require(_categoryToPAWNFTList[_newCategoryId].add(_tokenId), "OGNFT::updateTokenCategory::duplicated tokenId");

    emit SetTokenCategory(_tokenId, _newCategoryId);
  }

  /**
   * @dev Get the associated PAWName for a unique tokenId.
   */
  function getPAWNameOfTokenId(uint256 _tokenId) external view override returns (string memory) {
    return pawNames[_tokenId];
  }
  ...
  function mint(
    ...
    _tokenIds.increment();
    pawNFTToCategory[newId] = _categoryId;
    require(_categoryToPAWNFTList[_categoryId].add(newId), "OGNFT::mint::duplicated tokenId");
   ...
   */
  function setPAWName(uint256 _tokenId, string calldata _name) external onlyGovernance {
    string memory _prevName = pawNames[_tokenId];
    pawNames[_tokenId] = _name;

    emit SetPAWName(_tokenId, _prevName, _name);
  }
  ...
  function _withdrawFromMasterPAW(uint256 _categoryId, uint256 _shares) internal {
    if (_shares == 0) return;
    masterPAW.withdraw(_msgSender(), address(ogOwnerToken[_categoryId]), _shares);
  }
  ...
  function _harvestFromMasterPAW(address _user, uint256 _categoryId) internal {
    ...
    uint256 beforeReward = paw.balanceOf(_user);
    masterPAW.harvest(_user, stakeToken);

    emit Harvest(_user, _categoryId, paw.balanceOf(_user).sub(beforeReward));
  }
  ...
  function _stake(uint256 _tokenId, address _for) internal {
    uint256 categoryId = pawNFTToCategory[_tokenId];
    IOGOwnerToken stakeToken = ogOwnerToken[categoryId];
    _userStakeTokenIds[categoryId][_for].add(_tokenId);

    _harvestFromMasterPAW(_for, categoryId);
    stakeToken.mint(address(this), 1 ether);
    IERC20Upgradeable(address(stakeToken)).safeApprove(address(masterPAW), 1 ether);

    masterPAW.deposit(_for, address(stakeToken), 1 ether);

    IERC20Upgradeable(address(stakeToken)).safeApprove(address(masterPAW), 0);
  ...
  function _unstake(uint256 _tokenId) internal {
    ...
    _withdrawFromMasterPAW(categoryId, 1 ether);
  ...
  function harvest(uint256 _categoryId) external whenNotPaused nonReentrant {
    ...
    _harvestFromMasterPAW(_msgSender(), _categoryId);
  }
  ...
  function harvest(uint256[] calldata _categoryIdParams) external whenNotPaused nonReentrant {
    for (uint256 i = 0; i < _categoryIdParams.length; i++) {
      ...
      _harvestFromMasterPAW(_msgSender(), _categoryIdParams[i]);
    }
  }
