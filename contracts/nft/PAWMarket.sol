...
contract PAWMarket is ERC721HolderUpgradeable, OwnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable {
  ...
  // PAW original nft related
  mapping(address => mapping(uint256 => PAWNFTMetadata)) public pawNFTMetadata;
  ...
  modifier onlySupportedNFT(address _nft) {
    require(isNFTSupported[_nft], "PAWMarket::onlySupportedNFT::unsupported nft");
    _;
  }

  /// @notice only GOVERNANCE ROLE (role that can setup NON sensitive parameters) can continue the execution
  modifier onlyGovernance() {
    require(hasRole(GOVERNANCE_ROLE, _msgSender()), "PAWMarket::onlyGovernance::only GOVERNANCE role");
    _;
  }
  ...
  modifier withinBlockRange(address _nftAddress, uint256 _categoryId) {
    require(
      block.number >= pawNFTMetadata[_nftAddress][_categoryId].startBlock &&
        block.number <= pawNFTMetadata[_nftAddress][_categoryId].endBlock,
      "PAWMarket::withinBlockRange:: invalid block number"
    );
    _;
  }
  ...
  modifier onlyBiddingNFT(address _nftAddress, uint256 _categoryId) {
    require(
      pawNFTMetadata[_nftAddress][_categoryId].isBidding,
      "PAWMarket::onlyBiddingNFT::only bidding token can be used here"
    );
    _;
  }
  ...
  modifier onlyNonBiddingNFT(address _nftAddress, uint256 _categoryId) {
    require(
      !pawNFTMetadata[_nftAddress][_categoryId].isBidding,
      "PAWMarket::onlyNonBiddingNFT::only selling token can be used here"
    );
    _;
  }

  /// @dev set PAW NFT metadata consisted of cap, startBlock, and endBlock
  function setPAWNFTMetadata(PAWNFTMetadataParam[] calldata _params) external onlyGovernance {
    for (uint256 i = 0; i < _params.length; i++) {
      require(isNFTSupported[_params[i].nftAddress], "PAWMarket::setPAWNFTMetadata::unsupported nft");
      _setPAWNFTMetadata(_params[i]);
    }
  }

  function _setPAWNFTMetadata(PAWNFTMetadataParam memory _param) internal {
    require(
      _param.startBlock > block.number && _param.endBlock > _param.startBlock,
      "PAWMarket::_setPAWNFTMetadata::invalid start or end block"
    );
    PAWNFTMetadata storage metadata = pawNFTMetadata[_param.nftAddress][_param.nftCategoryId];
    metadata.cap = _param.cap;
    metadata.startBlock = _param.startBlock;
    metadata.endBlock = _param.endBlock;

    emit SetPAWNFTMetadata(_param.nftAddress, _param.nftCategoryId, _param.cap, _param.startBlock, _param.endBlock);
  }
  ...
  function buyBatchNFT(
    ...
    PAWNFTMetadata memory metadata = pawNFTMetadata[_nftAddress][_categoryId];
    ...
    require(
      ...
      "PAWMarket::buyBatchNFT:: invalid block number"
    );
  ...
  function _decreaseCap(
    ...
    require(pawNFTMetadata[_nftAddress][_categoryId].cap >= _size, "PAWMarket::_decreaseCap::maximum mint cap reached");
    pawNFTMetadata[_nftAddress][_categoryId].cap = pawNFTMetadata[_nftAddress][_categoryId].cap.sub(_size);
  }
  ...
  function _buyNFTTo(
    ...
    IPAWNFT(_nftAddress).mintBatch(_to, _categoryId, "", _size);
    emit Trade(
  ...
  function _setCurrentPrice(
    ...
    require(address(_quoteToken) != address(0), "PAWMarket::_setCurrentPrice::invalid quote token");
    pawNFTMetadata[_nftAddress][_categoryId].price = _price;
    pawNFTMetadata[_nftAddress][_categoryId].quoteBep20 = _quoteToken;
  ...
  function readyToStartAuction(
    ...
    pawNFTMetadata[_nftAddress][_categoryId].isBidding = true;
    _readyToSellNFTTo(
      _nftAddress,
      _categoryId,
      _price,
  ...
  function _readyToSellNFTTo(
    ...
    require(pawNFTMetadata[_nftAddress][_categoryId].startBlock == 0, "PAWMarket::_readyToSellNFTTo::duplicated entry");
    ...
    onlyBiddingNFT(_nftAddress, _categoryId)
  {
    ...
    require(bidEntry.bidder == address(0), "PAWMarket::cancelBiddingNFT::auction already has a bidder");
  ...
  function _cancelSellNFT(address _nftAddress, uint256 _categoryId) internal {
    ...
    delete pawNFTMetadata[_nftAddress][_categoryId];
  }
  ...
  function _bidNFT(
    ...
    require(_seller != _to, "PAWMarket::_bidNFT::Owner cannot bid");
    require(
      pawNFTMetadata[_nftAddress][_categoryId].price < _price,
      "PAWMarket::_bidNFT::price cannot be lower than or equal to the starting bid"
    );
    if (tokenBid[_nftAddress][_categoryId].bidder != address(0)) {
      require(
        tokenBid[_nftAddress][_categoryId].price < _price,
        "PAWMarket::_bidNFT::price cannot be lower than or equal to the latest bid"
      );
    }
    ...
    if (prevBid.bidder != address(0)) {
      _safeUnwrap(pawNFTMetadata[_nftAddress][_categoryId].quoteBep20, prevBid.bidder, prevBid.price);
    }
    _safeWrap(pawNFTMetadata[_nftAddress][_categoryId].quoteBep20, _price);
  ...
  function _concludeAuction(address _nftAddress, uint256 _categoryId) internal {
    require(
      block.number >= pawNFTMetadata[_nftAddress][_categoryId].endBlock,
      "PAWMarket::_concludeAuction::Unable to conclude auction now, bad block number"
    );
    address _seller = tokenCategorySellers[_nftAddress][_categoryId];
    _decreaseCap(_nftAddress, _categoryId, 1);
    BidEntry memory bidEntry = tokenBid[_nftAddress][_categoryId];
    require(bidEntry.price != 0, "PAWMarket::_concludeAuction::Bidder does not exist");
    uint256 price = bidEntry.price;
    uint256 feeAmount = price.mul(feePercentBps).div(1e4);
    _delBidByCompositeId(_nftAddress, _categoryId);
    if (feeAmount != 0) {
      pawNFTMetadata[_nftAddress][_categoryId].quoteBep20.safeTransfer(feeAddr, feeAmount);
    }
    pawNFTMetadata[_nftAddress][_categoryId].quoteBep20.safeTransfer(_seller, price.sub(feeAmount));
    IPAWNFT(_nftAddress).mint(bidEntry.bidder, _categoryId, "");
    emit Trade(
      tokenCategorySellers[_nftAddress][_categoryId],
      bidEntry.bidder,
      _nftAddress,
  ...
  function _safeWrap(IERC20Upgradeable _quoteBep20, uint256 _amount) internal {
    if (msg.value != 0) {
      require(address(_quoteBep20) == wNative, "PAWMarket::_safeWrap:: baseToken is not wNative");
      require(_amount == msg.value, "PAWMarket::_safeWrap:: value != msg.value");
