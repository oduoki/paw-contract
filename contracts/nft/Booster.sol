    ...
    IERC721Receiver
{
    ...
    bytes32 public constant SIGNATURE_HASH =
        0x08367bb0e0d2abf304a79452b2b95f4dc75fda0fc6df55dca6e5ad183de10cf0;
    ...
    mapping(address => mapping(address => NFTStakingInfo))
        public userStakingNFT;
    ...
    event Stake(
        address indexed staker,
        IERC20Upgradeable indexed stakeToken,
        uint256 amount
    );
    event Unstake(
    ...
    modifier inExec() {
        require(
            _IN_EXEC_LOCK == _NOT_ENTERED,
            "Booster::inExec:: in exec lock"
        );
        require(
            address(masterPAW) == _msgSender(),
            "Booster::inExec:: not from the master barista"
        );
    ...
    modifier isStakeTokenOK(address _stakeToken) {
        require(
            boosterConfig.stakeTokenAllowance(_stakeToken),
    ...
    modifier isBoosterNftOK(
        ...
        require(
            boosterConfig.boosterNftAllowance(
                _stakeToken,
                _nftAddress,
                _nftTokenId
            ),
    ...
    modifier permit(bytes calldata _sig) {
        address recoveredAddress = ECDSA.recover(
            ECDSA.toEthSignedMessageHash(SIGNATURE_HASH),
            _sig
        );
        require(
            recoveredAddress == _msgSender(),
    ...
    modifier onlyGovernance() {
        require(
            ...
            "Booster::onlyGovernance::only GOVERNANCE role"
        );
        _;
    }
    ...
    function _withdrawFromMasterPAW(
    ...
    /// @dev This one is a preparation for nft staking info, if nft address and nft token id are the same with existing record, it will be reverted
    /// @param _stakeToken a specified stake token address
    function unstakeNFT(address _stakeToken)
        external
        whenNotPaused
        isStakeTokenOK(_stakeToken)
        nonReentrant
    ...
    function _unstakeNFT(address _stakeToken) internal {
        ...
        require(
            toBeSentBackNft.nftAddress != address(0),
            "Booster::stakeNFT:: no nft staked"
        );
    ...
    /// @param _stakeToken a specified stake token to be unstaked
    function unstakeAll(address _stakeToken)
        external
        whenNotPaused
        isStakeTokenOK(_stakeToken)
        nonReentrant
    ...
    function harvest(address _stakeToken)
        external
        whenNotPaused
        isStakeTokenOK(_stakeToken)
        nonReentrant
    ...
    function harvest(address[] calldata _stakeTokens)
        external
        whenNotPaused
        nonReentrant
    {
        for (uint256 i = 0; i < _stakeTokens.length; i++) {
            require(
                boosterConfig.stakeTokenAllowance(_stakeTokens[i]),
                "Booster::harvest::bad stake token"
            );
            _harvestFromMasterPAW(
                _msgSender(),
                IERC20Upgradeable(_stakeTokens[i])
            );
        }
    }
    ...
    function _safeWrap(IERC20Upgradeable _quoteBep20, uint256 _amount)
        internal
    {
        if (msg.value != 0) {
            require(
                address(_quoteBep20) == wNative,
                "Booster::_safeWrap:: baseToken is not wNative"
            );
            require(
                _amount == msg.value,
                "Booster::_safeWrap:: value != msg.value"
            );
     ...
     * @dev EMERGENCY ONLY
     */
    function emergencyWithdraw(IERC20Upgradeable _stakeToken)
        external
        isStakeTokenOK(address(_stakeToken))
    {
        UserInfo storage user = userInfo[address(_stakeToken)][_msgSender()];
